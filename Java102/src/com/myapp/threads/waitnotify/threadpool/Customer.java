...
public class Customer implements Runnable
{
	...
	private void arrive()
	{
		...
		int timeBeforeArrival = Math.abs(tlr.nextInt())
				% (ApnaRestaurant.restaurantOpeningTime + 60);

		try
		{
			Thread.sleep(timeBeforeArrival
					* ApnaRestaurant.TIME_SCALER);
		}
	...
	public void run()
	{
		...
		ApnaRestaurant.numCustomersInRestaurant.incrementAndGet();

		// Bar is closed?
		if (Thread.interrupted() || ApnaRestaurant.closed)
		{
			// Come earlier next time
			System.out.println(name
					+ " has arrived after closing time - too late!");

			ApnaRestaurant.numCustomersInRestaurant.decrementAndGet();
